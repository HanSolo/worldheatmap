...
public class HeatMap extends ImageView {
     ...
     * @param Y
     */
    public void addEvent(final double X, final double Y) { addEvent(X, Y, eventImage, radius, radius); }
    ...
    public void updateMonochromeMap(final OpacityDistribution OPACITY_GRADIENT) {
        ...
        eventList.forEach(event -> {
            ...
            ctx.drawImage(createEventImage(event.getRadius(), OPACITY_GRADIENT), event.getX() - event.getRadius() * 0.5, event.getY() - event.getRadius() * 0.5);
        });
        updateHeatMap();
    }
