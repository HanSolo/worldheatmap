...
public class SimpleHeatMap {
    ...
    public void addEvents(final Point2D... EVENTS) { addEvents(Arrays.asList(EVENTS)); }
    public void addEvents(final List<Point2D> EVENTS) {
        for (Point2D event : EVENTS) { ctx.drawImage(eventImage, event.getX() - radius, event.getY() - radius); }
        updateHeatMap();
    }
    ...
    public double getHeatMapOpacity() { return heatMapView.getOpacity(); }
    public void setHeatMapOpacity(final double HEAT_MAP_OPACITY) {
        heatMapView.setOpacity(HEAT_MAP_OPACITY < 0 ? 0 : (HEAT_MAP_OPACITY > 1 ? 1 : HEAT_MAP_OPACITY));
    }
    ...
    public double getEventRadius() { return radius; }
    ...
    public void setSize(final double WIDTH, final double HEIGHT) {
        monochromeCanvas.resize(WIDTH, HEIGHT);
        if (WIDTH > 0 && HEIGHT > 0) {
            monochromeImage = new WritableImage(monochromeCanvas.widthProperty().intValue(), monochromeCanvas.heightProperty().intValue());
            updateHeatMap();
        }
    ...
    private double computeBrightness(final double RED, final double GREEN, final double BLUE) { return  (0.2126 * RED + 0.7152 * GREEN + 0.0722 * BLUE); }
    private double computeBrightnessFast(final double RED, final double GREEN, final double BLUE) { return ((RED + RED + BLUE + GREEN + GREEN + GREEN) / 6.0); }
    private double computePerceivedBrightness(final double RED, final double GREEN, final double BLUE) { return ((0.299 * RED) + (0.587 * GREEN) + (0.114 * BLUE)); }
    private double computePerceivedBrightnessFast(final double RED, final double GREEN, final double BLUE) { return ((RED + RED + RED  + BLUE + GREEN + GREEN + GREEN + GREEN) * 0.5); }

    private double computeLuminance(final double RED, final double GREEN, final double BLUE) { return Math.sqrt(0.241 * (RED * RED) + 0.691 * (GREEN * GREEN) + 0.068 * (BLUE * BLUE)); }
